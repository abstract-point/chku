...
CHKU_BACKUP_RETENTION_DAYS="${CHKU_BACKUP_RETENTION_DAYS:-14}"
...
infra/scripts/backup-db.sh "$TMP_SNAPSHOT/database/chku.dump"
infra/scripts/backup-storage.sh "$TMP_SNAPSHOT/storage/backend_storage_public.tar.gz"
...
echo "Backup snapshot saved to ${FINAL_SNAPSHOT}"
